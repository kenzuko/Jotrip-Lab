...
const fmt=(v,d=1)=>{if(v===null||v===undefined||Number.isNaN(Number(v)))return "-";const n=Number(v);if(Number.isInteger(n))return String(n);return n.toFixed(d).replace(/\.?0+$/,"")};
...
function renderPoint(){const p=state.points[currentPoint];$("pointName").textContent=p.name;$("pointStatus").textContent=p.status;for(const [id,key,d] of [["wind","wind",0],["gust","gust",0],["wave","wave",2],["period","period",2],["rain","rain",2],["current","current",2]])$(id).textContent=fmt(p[key],d);$("pointCaveat").textContent=p.caveat||"";renderTable(p.hours||[]);drawChart(p.hours||[])}

function renderTable(rows){
  $("forecastRows").innerHTML=rows.length?rows.map(r=>`<tr><td>${r.time}</td><td>${fmt(r.wind,0)}</td><td>${fmt(r.gust,0)}</td><td>${fmt(r.rain,2)}</td><td>${fmt(r.wave,2)}</td><td>${fmt(r.period,1)}</td></tr>`).join(""):`<tr><td colspan="6" style="text-align:center;color:#8da8ae">Chưa có full matrix 72 giờ</td></tr>`;
  $("forecastCards").innerHTML=rows.length?rows.map(r=>`<article class="forecast-card"><time>${r.time}</time><div class="forecast-card-grid"><div><span>Gió nền</span><b>${fmt(r.wind,0)} <small>km/h</small></b></div><div><span>Gió giật</span><b>${fmt(r.gust,0)} <small>km/h</small></b></div><div><span>Mưa 3 giờ</span><b>${fmt(r.rain,2)} <small>mm</small></b></div><div><span>Sóng Hs</span><b>${fmt(r.wave,2)} <small>m</small></b></div><div><span>Chu kỳ</span><b>${fmt(r.period,1)} <small>giây</small></b></div></div></article>`).join(""):`<div class="empty" style="display:grid">Chưa có full matrix 72 giờ.</div>`;
}
...
addEventListener("resize",()=>drawChart(state.points[currentPoint].hours||[]));
load();
