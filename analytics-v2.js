...
function delayed(r){const u=fold([r?.status_code,r?.status,r?.raw_status].join(' '));if(/DELAYED|RESCHEDULED|POSTPONED|TRE|HOAN|DOI GIO/.test(u))return true;if(Number(r?.delay_minutes)>0)return true;const s=r?.scheduled_time||r?.times?.[0],e=r?.estimated_time;return !!(s&&e&&s!==e&&diff(s,e)>0)}
...
function table(records){const map=new Map();for(const r of records||[]){const name=airline(r),k=fold(name)||'UNKNOWN';if(!map.has(k))map.set(k,{name,total:0,on:0,late:0,cancel:0,delaySum:0,delayN:0});const a=map.get(k);a.total++;if(cancelled(r)){a.cancel++;continue}if(delayed(r)){a.late++;const d=delayMinutes(r);if(Number.isFinite(d)){a.delaySum+=d;a.delayN++}}else a.on++}return [...map.values()].sort((a,b)=>b.total-a.total||b.on-a.on)}
