...
def snapshot(sampled: str, score_delta: int = 0, temp_delta: float = 0.0, level: str = "WATCH") -> dict:
    points = {}
    for i, point_id in enumerate(("an_thoi", "duong_dong", "ganh_dau", "rach_gia")):
        points[point_id] = {
            "lat": 10 + i / 10,
            "lon": 104 + i / 10,
...
class NowcastArchiveTests(unittest.TestCase):
    def test_daily_raw_plus_material_events(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(first["events_written"], 4)
            ...
            self.assertEqual(third["events_written"], 4)
            ...
            self.assertEqual(summary["material_event_count"], 8)
