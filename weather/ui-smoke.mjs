...
for(const [name,width,height] of [["history390",390,844],["historyDesktop",1440,1100]]){
  ...
  const checks=await page.evaluate(()=>({overflow:document.documentElement.scrollWidth-document.documentElement.clientWidth,archiveRows:document.querySelectorAll('.archive-row').length,compareCards:document.querySelectorAll('.compare-card').length,events:document.querySelectorAll('.event').length,search:!!document.querySelector('#historySearch'),dateA:!!document.querySelector('#dateA'),dateB:!!document.querySelector('#dateB'),point:!!document.querySelector('#comparePoint'),dataError:document.querySelector('#archiveHealth')?.classList.contains('bad')}));
  await page.screenshot({path:`artifacts/weather-ui-${name}.png`,fullPage:true});
