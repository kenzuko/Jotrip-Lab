...
REMAP_PACK_URL = ASSET_URL


def download_remap_pack(destination: Path) -> dict:
    ...
    with urllib.request.urlopen(request, timeout=120) as response, destination.open("wb") as stream:
        while chunk := response.read(1024 * 1024):
            ...
            digest.update(chunk)
            size += len(chunk)
    return {"source": ASSET_URL, "bytes": size, "sha256": digest.hexdigest()}
...
def inspect_official_pack(output: Path) -> dict:
    with tempfile.TemporaryDirectory(prefix="jotrip-icon-grid-") as temp:
        ...
        downloaded = download_remap_pack(archive)
