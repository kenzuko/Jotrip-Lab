...
function displayStatusLabel(r){
  ...
  if(/^(AIRBORNE|EN_ROUTE|IN_AIR)$/.test(code))return 'Đang bay';
...
function normalizedFlightNumber(r){return String(r?.operating_flight_number||'').toUpperCase().replace(/[^A-Z0-9]/g,'')}
function hasValidFlightNumber(r){const f=normalizedFlightNumber(r);return /^[A-Z0-9]{2,3}\d{1,4}[A-Z]?$/.test(f)}
function isVerifiedAirborne(r){
  const code=String(r?.status_code||'').toUpperCase();
  const text=[r?.status,r?.raw_status].filter(Boolean).join(' ').toUpperCase();
  return /^(AIRBORNE|EN_ROUTE|IN_AIR)$/.test(code)||/\bAIRBORNE\b|\bEN[ -]?ROUTE\b|\bIN[ -]?AIR\b|ĐANG BAY/.test(text);
}
function flightradarUrl(r){const f=normalizedFlightNumber(r);return f?`https://www.flightradar24.com/data/flights/${encodeURIComponent(f.toLowerCase())}`:''}
function fr24ActionHtml(r){
  if(!hasValidFlightNumber(r))return '';
  const live=isVerifiedAirborne(r),url=flightradarUrl(r);
  const label=live?'Track live':'Check FR24';
  const note=live?'Nguồn hiện tại xác nhận chuyến đang bay. FR24 sẽ xác nhận vị trí theo phạm vi theo dõi của họ.':'Mở Flightradar24 để kiểm tra chuyến này có đang được theo dõi trực tiếp hay không.';
  return `<div class="drawer-actions"><a class="fr24-action ${live?'is-live':''}" href="${url}" target="_blank" rel="noopener noreferrer">${label} ↗</a><small>${escapeHtml(note)}</small></div>`;
}
function mobileFr24Html(r){
  if(r?.direction!=='arrival'||!hasValidFlightNumber(r))return '';
  const done=/ARRIVED|ON_BLOCK|CANCELLED/.test(String(r?.status_code||'').toUpperCase())||/ĐÃ HẠ CÁNH|BÃI ĐỖ|HỦY|CANCEL/.test(String(r?.status||'').toUpperCase());
  if(done)return '';
  return `<a class="mobile-fr24 ${isVerifiedAirborne(r)?'is-live':''}" href="${flightradarUrl(r)}" target="_blank" rel="noopener noreferrer" aria-label="Kiểm tra ${escapeHtml(r.operating_flight_number)} trên Flightradar24">FR24 ↗</a>`;
}
...
async function load(){
  ...
  try{
    ...
    console.error(e);$('#errorBox').textContent='Không đọc được dữ liệu Sun Airport lúc này. Trang không hiển thị số cũ giả làm dữ liệu live.';$('#errorBox').classList.remove('hidden');setHealth('bad','MẤT DỮ LIỆU','Không thể tải nguồn live hoặc snapshot dự phòng.');
...
function renderSummary(){const l=state.latest;$('#totalFlights').textContent=l.counts?.total??'-';$('#arrivalsCount').textContent=l.counts?.arrivals??'-';$('#departuresCount').textContent=l.counts?.departures??'-';$('#internationalCount').textContent=l.summary?.arrivals_market?.international??'-';const age=ageInfo(l.collected_at_vn);const sourceText=state.dataSource==='live'?'JoTrip Live API':state.dataSource==='fallback'?'AutoSync fallback':'JoTrip AutoSync';$('#updatedAt').textContent=`Cập nhật ${new Date(l.collected_at_vn).toLocaleTimeString('vi-VN',{hour:'2-digit',minute:'2-digit',timeZone:'Asia/Ho_Chi_Minh'})} · ${age.label} trước · ${sourceText} · tự làm mới 1 phút`;const sl=$('.source-label');if(sl)sl.textContent=state.dataSource==='live'?'Sun Airport · JoTrip Live API':'Sun Airport · JoTrip AutoSync';const abnormal=(l.records||[]).filter(r=>!isPastRecord(r)&&changed(r)).length;const ops=$('#opsState');ops.className='ops-state';if(age.level==='stale'||age.level==='bad'){ops.textContent='DATA STALE';ops.classList.add('bad')}else if(abnormal>0||age.level==='watch'){ops.textContent='WATCH · '+abnormal+' BẤT THƯỜNG';ops.classList.add('watch')}else ops.textContent='OPERATIONS NORMAL'}
function renderHealth(){const l=state.latest,h=state.health,age=ageInfo(l.collected_at_vn);const qa=!!(h?.collector_completed&&h?.parser_passed&&h?.normalization_passed&&h?.qa_passed&&l?.quality?.usable);$('#qaText').textContent=qa?'PASS':'FAIL';$('#qaBadge').textContent=qa?'QA PASS':'QA FAIL';$('#qaBadge').className='qa-badge '+(qa?'good':'bad');$('#dataAge').textContent=age.label;let title,desc,level=age.level;if(!qa){level='bad';title='QA FAILED';desc='Nguồn hoặc normalization chưa đạt. Không nên dùng số liệu để điều hành.'}else if(age.level==='good'){title='GOOD · dữ liệu đang mới';desc=state.dataSource==='live'?'JoTrip Live API đang đọc trực tiếp API chính thức Sun Airport, có cache ngắn để giữ ổn định.':'Snapshot JoTrip AutoSync đã qua normalization và QA.'}else if(age.level==='watch'){title='WATCH · dữ liệu chậm cập nhật';desc=state.dataSource==='live'?'Nguồn live đang có độ trễ cao bất thường. Trang tự kiểm tra lại mỗi 1 phút.':'Snapshot vẫn hợp lệ nhưng đã quá 8 phút.'}else{title='STALE · không còn là dữ liệu live';desc='Dữ liệu đã quá 15 phút. Không nên xem đây là trạng thái tức thời của sân bay.'}if(state.dataSource==='fallback'&&qa){level=age.level==='stale'?'stale':'watch';title='FALLBACK · đang dùng AutoSync';desc='JoTrip Live API tạm không phản hồi. Giao diện đã tự chuyển sang snapshot dự phòng gần nhất.'}setHealth(level,title,desc)}
...
const extraStyle=document.createElement('style');extraStyle.textContent=`.search-wrap{margin:0 0 12px}.search-wrap input{height:48px;padding-left:42px;background:#f9fcfd;border-color:#d8e6ee}.search-wrap{position:relative}.search-wrap:before{content:'⌕';position:absolute;left:15px;top:9px;font-size:24px;color:#72869a;z-index:1}.status-pill.amber{font-weight:950}.mobile-fr24{display:none}.drawer-actions{display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin:-4px 0 18px}.fr24-action{display:inline-flex;align-items:center;justify-content:center;min-height:38px;padding:0 13px;border:1px solid #d6e4ec;border-radius:12px;background:#f5f9fb;color:#166f9f;text-decoration:none;font-size:11px;font-weight:900;letter-spacing:.01em}.fr24-action.is-live{background:#e8f8ee;border-color:#c9ead6;color:#147846}.drawer-actions small{flex:1 1 190px;color:#758496;font-size:10.5px;line-height:1.4}@media(max-width:680px){.search-wrap input{font-size:16px;height:48px}.status-pill{max-width:150px;overflow:hidden;text-overflow:ellipsis}.mobile-fr24{display:inline-flex;grid-column:2;grid-row:3;justify-self:start;align-items:center;min-height:28px;padding:0 8px;border:1px solid #d6e4ec;border-radius:9px;background:#f5f9fb;color:#166f9f;text-decoration:none;font-size:9.5px;font-weight:900;line-height:1}.mobile-fr24.is-live{background:#e8f8ee;border-color:#c9ead6;color:#147846}.flight-row:has(.mobile-fr24){padding-bottom:10px}.drawer-actions{margin-bottom:16px}.fr24-action{min-height:42px;padding:0 14px}.drawer-actions small{flex-basis:100%}}`;document.head.appendChild(extraStyle);
